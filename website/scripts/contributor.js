...
async function fetchData() {
	try {
		const contributorsResponse = await fetch(
			"https://api.github.com/repos/multiverseweb/Dataverse/contributors"
		);
...
function renderStats(repoStats, contributers) {
	...
	const stats = [
		{ label: "Contributors", value: contributers?.length, icon: "users" },
		{
			label: "Total Contributions",
			value:
				contributers?.reduce((sum, contributor) => sum + contributor.contributions, 0) || 0,
			icon: "git-commit"
		},
