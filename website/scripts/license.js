...
function updateIndicator(button) {
	...
	if (currentTheme === "light") {
		indicator.style.backgroundImage =
			"radial-gradient(circle, rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0) 70%)"; // Black glow in light mode
		indicator.style.boxShadow = "none"; // No box-shadow needed for glow
	} else {
		indicator.style.backgroundImage =
			"radial-gradient(circle, rgba(255, 255, 255, 0.8), rgba(255, 255, 255, 0) 70%)"; // White glow in dark mode
		indicator.style.boxShadow = "none"; // No box-shadow needed for glow
	}
}
